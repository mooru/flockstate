...
When the widget is used as a filter (e.g. in Views or in combination with Views
Global Filter) the slider automatically becomes a RANGE slider with two sliding
handles to set "from" and "to" range values. If the edit field is configured to
be displayed, it will be read-only.
